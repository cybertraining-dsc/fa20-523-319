...
## Abstract
Chest X-rays reveal many diseases. Early detection of a disease often improves the survival chance for Patients. It is one of the important tool for Radiologists to detect and identify underlying health conditions. But they are two major drawbacks. First, it takes time to analyze a radiograph. Second,  Radiologists make errors. Whether its an error in diagnosis or delay in diagnosis, both of these outcomes result in a loss of life. With the technological advances in AI, Deep Learning models address these drawbacks. The  Deep Learning models analyze the X-rays like a Radiologist and accurately predict much better than the Radiologists. In our project, first, we develop Deep Learning model and train our model to use the labels for Atelectasis, Cardiomegaly, Consolidation, Edema, and Pleural Effusion that corresponds to 5 different diseases respectively. Second, we test our model's performance, which is, how well our model predicts the diseases. Finally, we visualize our model's performance using the AUC-ROC curve.
...
Deep Learning methods are becoming very reliable at achieving expert level performance using large labeled datasets. In our project, we develop a Deep Learning model to use CheXpert dataset, which is a large dataset that contains 224,316 chest radiographs of 65,240 patients, to detect and classify the following 5 pathologies: Atelectasis, Cardiomegaly, Consolidation, Edema, and Pleural Effusion. The dataset has 14 observations in radiology reports and captures uncertainties inherent in radiograph interpretation using uncertainty labels. We impute uncertainty labels with randomly selected Boolean values. We develop our Deep Learning model using PyTorch library which is an open source machine learning library based on the Torch library [^2]. It was primarily developed by Facebook's AI Research lab (FAIR) and used for applications such as Computer Vision and NLP applications. PyTorch supports Python and C++ interface. There are popular Deep Learning applications that are built using PyTorch which includes Tesla Autopilot, Uber's Pyro etc. [^4].
