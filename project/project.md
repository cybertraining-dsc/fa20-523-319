...
In this analysis, first, we begin with an overview of the PyTorch library and DenseNet. We cover DenseNet architecture and advantages over ResNet for Multi-Image classification problems. Second, we explain the CheXpert dataset and how the classifiers are labeled, including uncertainties. Next, we explain the basic definitions of the AUC-ROC curve and how it measures a model's performance. Finally, we cover how our Deep Learning model classifies pathologies and concludes with the model's performance and prediction of the pathologies.
